...
// --- QP DISTRIBUTION PDF (HTML SCRAPER - WYSIWYG) ---
function generateQPDistributionPDF() {
    const { jsPDF } = window.jspdf;
    
    // 1. Validation: Ensure HTML report exists
    const reportContainer = document.getElementById('report-output-area');
    // We look for pages generated by the QP Distribution HTML generator
    const pages = reportContainer ? reportContainer.querySelectorAll('.print-page') : [];

    if (pages.length === 0) {
        return alert("Please click 'Generate QP Distribution Report' to create the HTML view first.");
    }
    ...
    try {
        ...
        pages.forEach((page, pageIndex) => {
            ...
            // --- A. SCRAPE HEADER ---
            // The HTML generator creates a .print-header-group div
            ...
            // --- B. SCRAPE CONTENT BLOCKS ---
            // Iterate direct children to maintain order
            const children = Array.from(page.children).filter(el => !el.classList.contains('print-header-group'));

            children.forEach(el => {
                // 1. STREAM TITLE HEADER
                // Detected by class 'font-bold text-sm uppercase' usually
                if (el.classList.contains('font-bold') && el.classList.contains('uppercase') && el.innerText.includes('STREAM')) {
                ...
                // 2. QP CARD BLOCK
                // Detected by internal grid or border style
                else if (el.querySelector('.grid')) {
                    
                    // -- Extract Data --
                    const headerRow = el.children[0]; // Title/QP/Total row
                    const gridRow = el.children[1];   // Room grid

                    // Course Name
                    const courseDiv = headerRow.querySelector('.font-bold.text-xs');
                    let courseName = courseDiv ? courseDiv.innerText.trim() : "";
                    
                    // QP Code
                    const qpBadge = headerRow.querySelector('span.bg-white.border-black');
                    const qpCode = qpBadge ? qpBadge.innerText.trim() : "N/A";

                    // Stream Badge (if exists)
                    const strmBadge = headerRow.querySelector('span.text-\\[9px\\]');
                    const strmLabel = strmBadge ? strmBadge.innerText.trim() : "";

                    // Total Count
                    const totalDiv = headerRow.querySelector('.text-right span');
                    const totalCount = totalDiv ? totalDiv.innerText.trim() : "";

                    // Rooms
                    const roomDivs = gridRow ? gridRow.querySelectorAll('.border.rounded') : [];
                    
                    // -- Determine Style (Regular vs Others) --
                    // Check background color or border style from HTML element
                    const isRegular = !el.style.border.includes("dashed");
                    
                    // -- Calculate Height --
                    const rows = Math.ceil(roomDivs.length / 3);
                    const cardHeight = 12 + (rows * 8) + 4; // Header + (Rows * RowH) + Padding
                    ...
                    if (!isRegular) {
                        ...
                        doc.rect(MARGIN, currentY, PAGE_W - (MARGIN*2), cardHeight, 'FD');
                    } else {
                        doc.rect(MARGIN, currentY, PAGE_W - (MARGIN*2), cardHeight);
                    }
                    ...
                    // Smart Truncate Course
                    if(doc.getTextWidth(courseName) > 130) courseName = courseName.substring(0, 75) + "...";
                    doc.text(courseName, MARGIN + 2, headY);
                    ...
                    let roomY = headY + 7;
                    let roomX = MARGIN + 2;
                    const boxW = (PAGE_W - (MARGIN*2) - 4) / 3;
                    ...
                    roomDivs.forEach((rDiv, idx) => {
                        ...
                        const roomNameTxt = rDiv.querySelector('.text-sm')?.innerText.trim() || "";
                        const locTxt = rDiv.querySelector('.text-gray-500.truncate')?.innerText.trim() || "";

                        // Draw Room Box
                        doc.setDrawColor(180); doc.setFillColor(255);
                        doc.rect(roomX, roomY, boxW - 2, 6, 'FD');

                        // Text
                        doc.setFontSize(9); doc.setFont("helvetica", "bold");
                        doc.text(countTxt, roomX + 2, roomY + 4.5);
                        doc.setFontSize(6); doc.setFont("helvetica", "normal");
                        doc.text("Nos", roomX + 8, roomY + 4.5);

                        // Vertical Separator
                        doc.line(roomX + 14, roomY + 1, roomX + 14, roomY + 5);

                        // Room Name
                        doc.setFontSize(8); doc.setFont("helvetica", "bold");
                        doc.text(roomNameTxt, roomX + 16, roomY + 4.5);

                        // Loc
                        if(locTxt) {
                            doc.setFontSize(6); doc.setFont("helvetica", "normal"); doc.setTextColor(80);
                            let cleanLoc = locTxt.replace(/[()]/g, '');
                            if(cleanLoc.length > 15) cleanLoc = cleanLoc.substring(0, 12) + "..";
                            doc.text(cleanLoc, roomX + 35, roomY + 4.5);
                            doc.setTextColor(0);
                        }

                        // Square Checkbox
                        doc.setDrawColor(0);
                        doc.rect(roomX + boxW - 7, roomY + 1.5, 3, 3);

                        roomX += boxW;
                    });

                    currentY += cardHeight + 2; // Move down for next card
                }
            });
        });

        const dateStr = new Date().toISOString().slice(0,10);
        doc.save(`QP_Distribution_Scraped_${dateStr}.pdf`);

    } catch (e) {
        console.error("PDF Scraper Error:", e);
        alert("Error creating PDF from HTML: " + e.message);
    } finally {
        if(btn) { btn.disabled = false; btn.innerHTML = "📄 Download PDF"; }
    }
}
