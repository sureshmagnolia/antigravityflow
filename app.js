...
//--------------------Generate Room Stickers (2 per Page)--------------

// --- ROOM STICKERS PDF (2 Per Page - HTML Scraper) ---
function generateRoomStickersPDF() {
    const { jsPDF } = window.jspdf;
    
    // 1. Validation
    const reportContainer = document.getElementById('report-output-area');
    // Look for the specific sticker pages generated by the HTML button
    ...
    try {
        ...
        const PAGE_H = 297;
        const MARGIN_X = 10;
        const STICKER_H = 135; // Fixed height from your CSS
        const STICKER_W = PAGE_W - (MARGIN_X * 2);
        
        // Y Positions for the two stickers
        const TOP_Y = 10;
        const BOT_Y = 10 + STICKER_H + 10; // 10mm gap

        pages.forEach((pageEl, pageIndex) => {
            if (pageIndex > 0) doc.addPage();

            // Find the 2 stickers on this page
            ...
            stickers.forEach((stickerEl, sIndex) => {
                ...
                // --- 1. DRAW BORDER ---
                doc.setDrawColor(0); doc.setLineWidth(0.4); doc.setLineDash([2, 2], 0); // Dashed
                doc.rect(MARGIN_X, startY, STICKER_W, STICKER_H);
                doc.setLineDash([]); // Reset
                doc.setLineWidth(0.1);

                // --- 2. SCRAPE HEADER ---
                // Structure: div (text-center) -> h1, div(date), div(room)
                const headerDiv = stickerEl.firstElementChild; // The top div
                ...
                doc.rect((PAGE_W/2) - 40, y, 80, 8); // Centered box
                doc.setFontSize(12); doc.setFont("helvetica", "bold");
                doc.text(roomSpan, PAGE_W / 2, y + 5.5, { align: 'center' });
                y += 12;

                // --- 3. SCRAPE BODY (Course Blocks) ---
                // The body is the second child div
                ...
                courseBlocks.forEach(block => {
                    // Header: Course Name + Count
                    const blockHeader = block.firstElementChild; 
                    // Extract text parts. Inner text is like "COURSE NAME 15"
                    // We can parse the DOM to be safer
                    ...
                    doc.rect(MARGIN_X + 2, currentBlockY, STICKER_W - 4, 6, 'S'); // Border
                    ...
                    const col1W = 10; // Seat
                    const col2W = 30; // Reg
                    const col3W = (STICKER_W - 4 - col1W - col2W) / 3; // Name (split into 3 cols effectively)
                    
                    // Actually the HTML grid is: Seat | Reg | Name
                    // And the main grid is 3 columns of STUDENTS.
                    // Let's re-read the HTML logic:
                    // <div style="display: grid; grid-template-columns: 1fr 1fr 1fr ..."> 
                    //    <div style="display: grid ...">Student</div> ...
                    // </div>
                    
                    // We need to calculate row height. HTML determines layout. 
                    // Since scraping exact grid positions is hard, we'll re-flow them into 3 columns on PDF.
                    
                    const cellW = (STICKER_W - 6) / 3; // 3 Students per row
                    let colIndex = 0;
                    ...
                    studentRows.forEach(rowEl => {
                        ...
                        // Draw text
                        doc.setFont("helvetica", "bold");
                        doc.text(seat, xBase + 2, rowY + 4); // Seat
                        
                        doc.setFont("helvetica", "normal");
                        doc.text(reg, xBase + 10, rowY + 4); // Reg
                        ...
                        doc.line(xBase, rowY + 6, xBase + cellW - 2, rowY + 6);
                    ...
                    // Advance Y for next block (add remaining row height if partial row)
                ...
                doc.line(MARGIN_X, footerY, MARGIN_X + STICKER_W, footerY); // Top line
