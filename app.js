...
let lastGeneratedReportType = ""; // V91: To track the type for PDF filename
...
// const downloadPdfButton = document.getElementById('download-pdf-button'); // V96: Removed
...
const navRoomAllotment = document.getElementById('nav-room-allotment'); // Room Allotment
const viewRoomAllotment = document.getElementById('view-room-allotment'); // Room Allotment
const allNavButtons = [navExtractor, navSettings, navRoomSettings, navQPCodes, navRoomAllotment, navReports, navAbsentees]; // V83
const allViews = [viewExtractor, viewSettings, viewRoomSettings, viewQPCodes, viewRoomAllotment, viewReports, viewAbsentees]; // V83

// --- (V26) Get references to NEW Room Settings elements ---
const collegeNameInput = document.getElementById('college-name-input'); // V48
// V97: New Status and Button
...
let currentSessionAllotment = []; // Array of {roomName, capacity, students: []}
...
function performRoomAllocation(data) {
    ...
    for (const row of data) {
        ...
        // 3a. Check if manual allotment exists for this session
        ...
        // 3b. If no manual allotment, use automatic allocation
        if (assignedRoomName === "") {
...
generateReportButton.addEventListener('click', async () => {
    ...
    try {
        ...
        lastGeneratedReportType = "Roomwise_Seating_Report"; // V91: Set report type
...
navRoomAllotment.addEventListener('click', () => showView(viewRoomAllotment, navRoomAllotment)); // Room Allotment
...
function selectRoomForAllotment(roomName, capacity) {
    ...
    // Get unallotted students
    const unallottedStudents = sessionStudents.filter(s => !allottedRegNos.has(s['Register Number']));
