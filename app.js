...
// 3. Delete Logic (Wipes Students + Associated Data)
    ...
    window.openBatchArchiveModal = function() {
        ...
        if (known.length === 0) {
            ...
            known.sort((a, b) => {
                const dateA = parseInt(a.split('.')[2] + a.split('.')[1] + a.split('.')[0]); 
                const dateB = parseInt(b.split('.')[2] + b.split('.')[1] + b.split('.')[0]);
                return dateB - dateA;
            });
        ...
        document.getElementById('batch-archive-modal').classList.remove('hidden');
    };
    ...
    window.generateBatchArchive = async function() {
        ...
        checked.forEach(sessionKey => {
            const [date, time] = sessionKey.split(' | ');
            const students = allStudentData.filter(s => s.Date === date.trim() && s.Time === time.trim());
